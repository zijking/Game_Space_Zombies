...
(function ($, window, undefined) {
                ...
                $this.data('animateSprite', {
                    ...
                    controlAnimation: function () {
                        ...
                        if (this.settings.animations === undefined) {
                            ...
                            //console.log(this.currentAnimation[this.currentFrame] + " ----- " + this.currentAnimation[this.currentFrame]);
                            var newFrame = this.currentAnimation[this.currentFrame];

                            //   console.log($this.css('background-image') + " newFrame " + newFrame);
                    ...
                    controlTimer: function () {
                        ...
                        if (data.settings.duration !== undefined) {
                           ...
                           // console.log("speed " + speed   + " data.settings.duration " + data.settings.duration + " data.settings.totalFrames " + data.settings.totalFrames);
                        }
    ...
    var frame = function (frameNumber) {
        ...
        return this.each(function () {
            if ($(this).data('animateSprite') !== undefined) {
                ...
                //  console.log(frameNumber + "  " + -data.settings.width * column + " " + -data.settings.height * row + " data.settings.columns  " + data.settings.columns);
    ...
    var stop = function () {
        return this.each(function () {
            var $this = $(this),
                data = $this.data('animateSprite');
            clearTimeout(data.currentFrame);
            data.currentFrame = -1;
    ...
    var play = function (animationName) {
        return this.each(function () {
            ...
            if (typeof animationName === 'string') {
                ...
                if (animationName == "slow" || animationName == "stop" || animationName == "slowx") {
                    data.isPlaying = false;
                    clearTimeout(data.interval);
                } else {
                    data.isPlaying = true;
    ...
    $.fn.animateSprite = function (method) {
        if (methods[method]) {
            ...
            // console.log('Method ' + method + ' does not exist on jQuery.animateSprite');
